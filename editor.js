...
(function (pxt) {
    ...
    (function (editor) {
        ...
        var DAPWrapper = (function () {
            function DAPWrapper(h) {
                this.packetIo = h;
                var pbuf = new pxt.U.PromiseBuffer();
                var sendMany = function (cmds) {
                    return h.talksAsync(cmds.map(function (c) { return ({ cmd: 0, data: c }); }));
                };
                if (!h.talksAsync)
                    sendMany = null;
        ...
        function dapAsync() {
            return Promise.resolve()
                ...
                .then(function () { return pxt.HF2.mkPacketIOAsync(); })
                .then(function (h) {
        ...
        function initAsync() {
            ...
            if (pxt.U.isNodeJS) {
                ...
                if (pxt.Cloud.isLocalHost() && pxt.Cloud.localToken && !forceHexDownload || isUwp)
                    canHID = true;
            }
            if (canHID) {
                return dapAsync();
        ...
        editor.bufferConcat = function (bufs) {
            var len = 0;
            for (var _i = 0, bufs_1 = bufs; _i < bufs_1.length; _i++) {
                var b = bufs_1[_i];
                len += b.length;
        ...
        function deployCoreAsync(resp, isCli) {
            if (isCli === void 0) { isCli = false; }
            var saveHexAsync = function () {
                if (isCli) {
                    return Promise.resolve();
                }
                else {
            ...
            return initAsync()
                ...
                .then(function () { return getFlashChecksumsAsync(wrap); })
                .then(function (buf) {
                checksums = buf;
                log("write code");
                ...
                var uf2 = UF2.newBlockFile();
                UF2.writeHex(uf2, resp.outfiles[pxtc.BINARY_HEX].split(/\r?\n/));
                var bytes = pxt.U.stringToUint8Array(UF2.serializeFile(uf2));
                var parsed = UF2.parseFile(bytes);
        ...
        editor.deployCoreAsync = deployCoreAsync;
